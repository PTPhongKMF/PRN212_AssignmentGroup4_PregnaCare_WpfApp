...
namespace PregnaCare_WpfApp
{
    ...
    public partial class UpdateBlogWindow : Window
    {
        ...
        {
            var existingBlog = _blogService.GetBlogById(_blogId);
            if (existingBlog != null)
            {
                txtPageTitle.Text = existingBlog.PageTitle;
                txtHeading.Text = existingBlog.Heading;
                txtShortDescription.Text = existingBlog.ShortDescription;
                txtContent.Text = existingBlog.Content;
                txtFeaturedImageUrl.Text = existingBlog.FeaturedImageUrl;
                chkIsVisible.IsChecked = existingBlog.IsVisible;
            }
        }
            ...
            private void btnSave_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                bool success = _blogService.UpdateBlog(updatedBlog);

                if (success)
                {
                    MessageBox.Show("Blog post update successfully!",
                                  "Success",
                                  MessageBoxButton.OK,
                ...
                {
                    MessageBox.Show("Failed to update blog post.",
                                  "Error",
                                  MessageBoxButton.OK,
                                  MessageBoxImage.Error);
                }
